...
지금까지 정리한 하네스 논의를 한 단계 더 시각화하면, Claude Code의 각 실행 모드는 결국 **노드(작업 단위)와 엣지(흐름)의 조합** 으로 환원됩니다. 같은 인터페이스 안에서도 일반 채팅, Skill, Subagent, Batch, Deep Research, Ultracode, `/goal`은 서로 다른 그래프 모양을 가지며, 이 그래프 모양 자체가 곧 하네스의 성격입니다. 단순 응답일수록 그래프는 선형이고, 깊은 추론·자율 실행일수록 루프와 fan-out이 등장합니다. 아래는 각 모드를 개별 그래프로 분리해 본 지도입니다.
