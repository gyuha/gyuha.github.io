...
## 3. 작업 루프가 더 넓어졌다: strategy → ideate → brainstorm → plan → work → review → compound
...
이 순서는 상당히 의도적입니다. 먼저 제품의 target problem, approach, persona, metrics를 `STRATEGY.md`로 고정하고, 그다음 큰 아이디어를 검토하고, 요구사항을 대화형으로 좁히고, 구현 계획으로 내리고, 작업을 수행하고, 멀티 에이전트 리뷰를 돌린 뒤, 마지막으로 교훈을 문서로 남깁니다. [README](https://github.com/EveryInc/compound-engineering-plugin)

여기에 `/ce-product-pulse`가 붙으면서 루프가 더 흥미로워졌습니다. 단순히 만드는 쪽만 보는 것이 아니라, 24시간·7일 같은 시간 창에서 실제 사용량, 성능, 에러, 후속 조치를 요약해 `docs/pulse-reports/`에 남깁니다. 즉 다음 전략 업데이트와 다음 brainstorm가 실제 사용자 신호를 읽고 시작할 수 있습니다.
...
flowchart LR
    ...
    G --> H["/ce-code-review"]
    H --> I["/ce-compound"]
    J["/ce-product-pulse"] --> B
    I --> K["다음 작업의 더 좋은 맥락"]
```

## 4. 중요한 것은 스킬 수보다 루프 완성도다

README에 따르면 현재 이 플러그인은 `37 skills` 와 `51 agents` 를 제공합니다. 숫자만 보면 거대한 스킬 번들처럼 보일 수 있습니다. [README](https://github.com/EveryInc/compound-engineering-plugin)
...
- `ce-debug` 는 실패 재현과 원인 추적을 맡고
- `ce-code-review` 는 품질을 점검하고
- `ce-product-pulse` 는 실제 사용 신호를 다음 전략으로 되돌리고
...
README는 설치 경로를 Claude Code, Cursor, Codex, GitHub Copilot, Factory Droid, Qwen Code, OpenCode, Pi, Gemini, Kiro 등 여러 도구에 대해 안내합니다. 특히 Codex에서는 마켓플레이스 등록, 에이전트 설치, TUI에서 플러그인 설치라는 3단계를 따로 설명할 정도로 자세합니다. [README](https://github.com/EveryInc/compound-engineering-plugin)

Codex에서 이 3단계가 필요한 이유도 중요합니다. 현재 Codex의 plugin spec이 custom agents를 완전히 등록하지 못하기 때문에, skills는 plugin marketplace/TUI로 설치하고 review·research·workflow agents는 Bun installer로 별도 설치해야 합니다. README는 Codex native plugin spec이 custom agents를 지원하게 되면 이 Bun step이 사라질 것이라고 설명합니다.
...
- strategy와 product pulse만 붙여 제품 방향과 실제 사용 신호를 연결하거나
- 버그 수정 흐름에서 `ce-debug → ce-code-review → ce-compound` 만 붙여 보는 방식입니다

즉 기능 전체보다 **루프의 어느 지점이 우리 팀의 병목인지** 를 먼저 찾아 붙이는 편이 좋습니다.

## 2026년 5월 업데이트: 이제는 제품 전략과 실제 사용 신호까지 루프에 들어왔다

처음 이 글을 썼을 때 핵심은 planning, review, compound였습니다.  
현재 README 기준으로는 범위가 더 넓어졌습니다.

- `/ce-strategy`: 제품의 target problem, approach, persona, key metrics, tracks를 `STRATEGY.md`로 유지
- `/ce-product-pulse`: usage, performance, errors, followups를 시간 창 기준으로 요약해 `docs/pulse-reports/`에 저장
- `/ce-debug`: 버그를 체계적으로 재현하고 root cause를 추적
- 37 skills / 51 agents
- Claude Code, Cursor, Codex, Copilot, Droid, Qwen, OpenCode, Pi, Gemini, Kiro까지 설치 경로 확대

이 변화는 작지 않습니다.

Compound Engineering Plugin이 단순히 “코딩 전에 계획하고, 코딩 후 리뷰하자”에서 멈추지 않고, **제품 전략 → 아이디어 → 요구사항 → 구현 → 리뷰 → 학습 → 실제 사용자 신호 → 다음 전략**으로 이어지는 더 긴 루프를 노리고 있다는 뜻이기 때문입니다.

## 핵심 요약

- `compound-engineering-plugin`은 스킬 번들보다 엔지니어링 루프에 가깝다.
- 핵심 철학은 `80% planning and review, 20% execution` 이다.
- `/ce-strategy`, `/ce-ideate`, `/ce-brainstorm`, `/ce-plan`, `/ce-work`, `/ce-debug`, `/ce-code-review`, `/ce-compound`, `/ce-product-pulse` 가 하나의 확장된 사이클을 이룬다.
- 진짜 차별점은 리뷰와 학습 축적 단계를 명시적으로 포함한다는 점이다.
- Claude Code, Cursor, Codex, GitHub Copilot, Droid, Qwen, OpenCode, Pi, Gemini, Kiro 등 여러 도구에 설치되는 것은 특정 모델보다 작업 방식이 더 중요하다는 뜻이다.
- 2026년 5월 12일 기준 저장소는 stars 16,591, forks 1,299, MIT 라이선스, TypeScript 프로젝트다.
