...
최근 AI 에이전트 생태계에서 Model Context Protocol(MCP)은 도구 연결의 표준으로 자리 잡았습니다. 하지만 모든 도구를 MCP로 연결하는 것이 항상 최선은 아닙니다. 특히 개발자 개인의 워크플로우에서는 MCP의 과도한 토큰 소모와 성능 저하가 문제가 될 수 있습니다. 이번 글에서는 VibeLabs의 분석과 여러 벤치마크 결과를 바탕으로, 왜 CLI와 Skills의 결합이 더 효율적인 대안이 될 수 있는지, 그리고 어떤 상황에서 MCP를 유지해야 하는지 살펴보겠습니다.
...
VibeLabs의 분석에 따르면, GitHub Copilot MCP 서버를 연결할 경우 약 43개의 도구가 노출됩니다. 문제는 에이전트가 실제 코드를 읽기 전부터 약 55,000개의 토큰이 컨텍스트에 주입된다는 점입니다. 이러한 도구 정의 비용은 세션 전체의 비용을 급격히 상승시킵니다.
...
Scalekit의 벤치마크 결과는 이를 뒷받침합니다. 75회의 테스트에서 CLI 방식은 효율성 지표에서 MCP를 압도했습니다. MCP는 테스트된 작업에서 CLI보다 4배에서 최대 32배 더 많은 토큰을 소모했으며, 신뢰도는 약 72% 수준에 머물렀습니다.
...
flowchart TD
    ...
    class Sub1,Sub2,Perf1,Perf2 default
```
...
flowchart TD
    ...
    class Task,Router,Result default
```
