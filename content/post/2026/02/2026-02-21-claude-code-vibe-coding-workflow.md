...
초기 설정 파일인 CLAUDE.md는 매 세션마다 로드되므로, 그 크기가 컨텍스트 효율성에 직결됩니다.

> [!NOTE]
> `CLAUDE.md`는 반드시 알아야 할 핵심 원칙과 명령어 위주로 `500줄(또는 2000 토큰)` 이하로 간결하게 유지해야 합니다.
...
> [!NOTE]
> 길고 상세한 API 가이드나 복잡한 워크플로우 등은 **스킬(Skills)** 로 분리해야 합니다. 스킬은 '점진적 공개(Progressive disclosure)' 방식을 사용하여, 세션 시작 시에는 이름과 설명만 로드되고 클로드가 해당 스킬이 필요하다고 판단할 때만 전체 내용이 컨텍스트로 불러와집니다.
