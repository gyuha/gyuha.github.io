...
draft: false
tags: [agents, git, automation]
description: "Git 공식 문서와 KKamJi 글을 바탕으로, git worktree를 실제로 언제 어떻게 쓰는지 예제 중심으로 정리합니다. 새 브랜치 작업, 긴급 hotfix, 병렬 멀티 에이전트 운영, 정리와 복구까지 한 흐름으로 다룹니다."
---
...
- [https://kkamji.net/posts/git-worktree/](https://kkamji.net/posts/git-worktree/)
- [Git Docs - git-worktree](https://git-scm.com/docs/git-worktree)
- [GitHub Blog - Highlights from Git 2.42](https://github.blog/open-source/git/highlights-from-git-2-42/#working-with-multiple-worktrees)
...
```mermaid
flowchart TD
    REPO["하나의 Git 저장소"] --> MAIN["원래 작업 디렉터리<br>예: main"]
    REPO --> FEAT["새 worktree<br>../feat-login"]
    ...
    class LOCAL1,LOCAL2 meta
```

이 예제를 한 번 써 보면, worktree를 "멀티 에이전트 전용 고급 기능"이 아니라 **브랜치별 작업공간을 분리하는 일상적인 도구** 로 받아들이게 됩니다. 그리고 이 감각이 있어야 뒤의 hotfix나 병렬 작업 예제도 쉬워집니다.
...
작업이 자주 끊기는 환경일수록 이 예제의 가치가 커집니다. AI 에이전트 세션을 오래 유지하는 경우에도 마찬가지입니다. 한 세션은 계속 피처 개발을 하고, 다른 세션은 hotfix만 처리한 뒤 바로 닫을 수 있기 때문입니다.
...
flowchart TD
    ...
    class DETACH,RESULT2 warn
```

정리하면, "읽기 전용 확인용" worktree와 "실제로 계속 작업할" worktree를 구분해서 생각하면 쉽습니다. 후자라면 로컬 브랜치와 upstream을 명시적으로 챙기는 것이 좋습니다.
...
이 패턴은 꼭 AI 에이전트가 아니어도 유효합니다. 사람 팀원끼리 병렬 작업할 때도 동일하게 적용됩니다. 다만 여러 에이전트를 붙일수록 의존성 설치는 각 worktree에서 따로 필요할 수 있다는 점은 꼭 기억해야 합니다. KKamJi 글도 `node_modules`, `venv` 같은 디렉터리는 worktree마다 별도 준비가 필요하다고 설명합니다.
...
flowchart TD
    ...
    DONE1 -->|"예"| REMOVE["git worktree remove <path>"]
    DONE1 -->|"아니오"| KEEP["유지"]
    ...
    class REMOVE,KEEP,PRUNE,REPAIR,CLEAN process
```

## 6) bare repository와 `wt` CLI는 언제 고려하면 좋을까
...
- 가장 먼저 익혀야 할 패턴은 `git worktree add ../feat-login -b feat/login main` 처럼 새 기능 작업을 별도 폴더로 분리하는 예제입니다.
- 공식 문서가 보여 주는 대표 사례는 긴급 hotfix를 새 worktree에서 처리하고, 끝나면 제거한 뒤 원래 작업으로 돌아오는 흐름입니다.
- 원격 브랜치를 worktree로 열 때는 `detached HEAD` 를 피하려고 로컬 브랜치와 upstream을 함께 챙기는 쪽이 실무적으로 안전합니다.
- 멀티 에이전트 환경에서 worktree의 진짜 가치는 저장소 복제가 아니라 **작업 디렉터리 분리와 컨텍스트 격리** 에 있습니다.
- `git worktree remove`, `prune`, `repair` 를 알고 있으면 많이 쓰는 환경에서도 정리와 복구가 수월합니다.
- bare repository와 `wt` 같은 래퍼 CLI는 worktree 사용량이 커진 뒤에 운영 표준화를 위해 도입하는 전략으로 보면 됩니다.

## 결론

`git worktree` 를 어렵게 느끼게 만드는 것은 개념 자체보다, 언제 어떤 예제로 감을 잡아야 하는지 감이 없기 때문입니다. 그래서 처음에는 이 글의 순서대로 **새 기능 분리 -> 긴급 hotfix -> 기존 브랜치 체크아웃 -> 병렬 작업 -> 정리와 복구** 순서로 직접 해 보는 것이 가장 좋습니다.<br>그 흐름만 몸에 익으면 worktree는 특수한 고급 기능이 아니라, 브랜치 작업을 더 안전하고 덜 번거롭게 만드는 기본 도구로 자리 잡게 됩니다.

<!--
Evidence notes
- claim: Git worktree lets one repository support multiple working trees and more than one branch checked out at a time | evidence quote: "A git repository can support multiple working trees, allowing you to check out more than one branch at a time." | url: https://git-scm.com/docs/git-worktree | confidence: high
- claim: A repository has one main worktree and zero or more linked worktrees | evidence quote: "A repository has one main worktree (if it's not a bare repository) and zero or more linked worktrees." | url: https://git-scm.com/docs/git-worktree | confidence: high
- claim: Linked worktrees share repository data except per-worktree files such as HEAD and index | evidence quote: "sharing everything except per-worktree files such as HEAD, index, etc." | url: https://git-scm.com/docs/git-worktree | confidence: high
- claim: In the simplest form, worktree add can create a new branch named after the final component of the path | evidence quote: "git worktree add ../hotfix creates new branch hotfix and checks it out at path ../hotfix" | url: https://git-scm.com/docs/git-worktree | confidence: high
- claim: A temporary linked worktree is a clean alternative to stash during interrupt-driven work | evidence quote: "Instead, you create a temporary linked worktree to make the emergency fix, remove it when done, and then resume your earlier refactoring session." | url: https://git-scm.com/docs/git-worktree | confidence: high
- claim: Remove and prune are the official cleanup commands when linked worktrees are finished or stale | evidence quote: "When you are done with a linked worktree, remove it with git worktree remove." and "you can run git worktree prune" | url: https://git-scm.com/docs/git-worktree | confidence: high
- claim: Manual movement or deletion of worktrees can require repair or prune workflows | evidence quote: "Better yet, run git worktree repair to reestablish the connection automatically." | url: https://git-scm.com/docs/git-worktree | confidence: high
- claim: Bare-repository-centered multi-agent layout is presented in the source article as a way to manage every branch symmetrically | evidence quote: "Bare repository 방식" and "모든 브랜치를 동등하게 worktree로 관리" | url: https://kkamji.net/posts/git-worktree/ | confidence: medium
- claim: The source article positions worktree as useful for multi-agent parallel work by assigning isolated worktrees to each agent | evidence quote: "각 AI 에이전트에게 독립된 worktree 할당 -> 컨텍스트 분리 및 충돌 위험 감소" | url: https://kkamji.net/posts/git-worktree/ | confidence: high
- claim: The article recommends keeping branch names and worktree paths aligned for traceability and automation | evidence quote: "브랜치 이름과 worktree 경로를 동일하게 맞추면 추적성과 자동화가 좋아집니다." | url: https://kkamji.net/posts/git-worktree/ | confidence: high
- claim: Detached HEAD can happen when adding a worktree directly from a remote branch without creating a local branch | evidence quote: "원격 브랜치를 직접 worktree로 추가하여 detached HEAD 상태가 됨" | url: https://kkamji.net/posts/git-worktree/ | confidence: high
- claim: Each worktree needs its own dependency installation because the filesystem is independent even though Git data is shared | evidence quote: "각 worktree는 독립된 파일 시스템이므로 각각 설치 필요" | url: https://kkamji.net/posts/git-worktree/ | confidence: high
- claim: The article presents wt as a CLI that automates bare repo and worktree setup with commands such as wt init and wt add | evidence quote: "wt(git-worktree-tool)는 이 과정을 자동화해주는 CLI 도구" | url: https://kkamji.net/posts/git-worktree/ | confidence: high
-->
